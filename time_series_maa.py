...
class MAA_time_series(GCABase):
    ...
    # 剥离数据中的交易日、收盘价、最高价、最低价、开盘价、成交量，用于回测
    @log_execution_time
    def Backtrade_data(self):
        """
        直接从 data_path 文件提取 test 区间的 trade_date、close、high、low、open、vol。
        """
        df = self.raw_data
        total_len = df.shape[0]
        test_start = int(df.shape[0] * self.train_split)
        test_end = total_len
        cols = ['trade_date', 'close', 'high', 'low', 'open', 'vol']
        return df.iloc[test_start:test_end][cols]
    ...
    def init_dataloader(self):
        ...
        for i, (x, y_gan, label_gan) in enumerate(
            ...
            dataloader = DataLoader(
                ...
                shuffle=False,  # shuffle_flag,  # 如果后面要计算return的话，就不能 shuffle
